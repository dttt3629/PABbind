#  The feature script highly refer with Zhang Xujun's work
...
def positional_embeddings_v1(edge_index,
    ...
    # new
    # It is too cost and waste the space
...
def get_protein_feature_mda(pocket_mol_path, top_k=30,chain_id=[]):
    chain_dic = {}
    # chain_id ['H','L','P1','P2'] or ['A1A2','','B3B4']
    for i,n in enumerate(chain_id):
        if i<2:
             for c in n: 
                   chain_dic[c]=i
        else:
            for c in n: 
                   chain_dic[c]=2
    # try:
    pocket_mol = mda.Universe(pocket_mol_path)
    with torch.no_grad():
        ...
        for res in pocket_mol.residues:
            # try:
                ...
                res_atoms = res.atoms
                sele=[]
                # quite time waste if pdbs are clean,cover it
                for a in res_atoms:
                      if a.altLoc=='A'or a.altLoc=='':
                        sele.append(a)
                res_atoms = mda.AtomGroup(sele)
                ...
                try:
                    ...
                    continue
                seg = res.segid
                # if seg in chain_dic.keys():
                segid.append(chain_dic[seg])
                ...
                resid.append((res.segid,res.resid))
        # assert 2 in segid
            # except:
            #     continue
        # node features
        ...
        # we dont use it here, btw, it pretty cost
        xyz_full = None
        ...
        full_edge_s= None
    # except:
    #     print(f'wrong in {pocket_mol_path}')
    return (X_ca, xyz_full, seq, node_s, node_v, edge_index, edge_s, edge_v, full_edge_s,segid,resid)

def get_protein_feature_se3(pocket_mol_path, top_k=30,chain_id=[]):
    chain_dic = {chain_id[0]:0,chain_id[1]:1}
    ...
    with torch.no_grad():
        ...
        for res in pocket_mol.residues:
            # try:
                ...
                ca = res_atoms.select_atoms("name CA")[0]
                ...
                node_s.append(intra_dis+obtain_dihediral_angles(res))
                pure_res_lis.append(res)
        assert 2 in segid,f'{pocket_mol_path}'
            # except:
            #     continue
        # node features
        seq = torch.from_numpy(np.asarray(seq))
    ...
    # except:
    #     print(f'wrong in {pocket_mol_path}')
    return (X_ca, xyz_full, seq, node_s, node_v, edge_index, edge_s, edge_v, full_edge_s,segid)
